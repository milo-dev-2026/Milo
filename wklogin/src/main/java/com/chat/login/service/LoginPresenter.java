...
public class LoginPresenter implements LoginContract.LoginPersenter {
    ...
    @Override
    public void emailRegisterCode(String email) {
        LoginModel.getInstance().emailRegisterCode(email, (code, msg, exist) -> {
            if (loginView.get() != null) {
                loginView.get().hideLoading();
                if (code == HttpResponseCode.success) {
                    loginView.get().setEmailRegisterCodeSuccess(code, msg);
                } else {
                    loginView.get().showError(msg);
                }
            }
    ...
    @Override
    public void emailRegisterApp(String code, String email, String password, String inviteCode) {
        LoginModel.getInstance().emailRegisterApp(code, email, password, inviteCode, (code1, errorMsg, userInfoEntity) -> {
            if (code1 == HttpResponseCode.success) {
                if (loginView.get() != null) loginView.get().loginResult(userInfoEntity);
            } else {
                if (loginView.get() != null) {
    ...
    @Override
    public void emailForgetPwd(String email) {
        LoginModel.getInstance().emailForgetPwd(email, (code, msg, exist) -> {
            if (loginView.get() != null) {
                loginView.get().hideLoading();
                if (code == HttpResponseCode.success) {
                    loginView.get().setEmailRegisterCodeSuccess(code, msg);
    ...
    @Override
    public void emailResetPwd(String email, String code, String pwd) {
        LoginModel.getInstance().emailResetPwd(email, code, pwd, (code1, errorMsg) -> {
            if (code1 == HttpResponseCode.success) {
    ...
    @Override
    public void isRegister(String zone, String phone, int accountType) {
        LoginModel.getInstance().isRegister(zone, phone, accountType, (code, msg, exist) -> {
            if (loginView.get() != null) {
                loginView.get().hideLoading();
                if (code == HttpResponseCode.success || code == 404) {
                    loginView.get().setAccountRegistrationResult(code, msg, exist);
                } else {
                    loginView.get().showError(msg);
                }
            }
