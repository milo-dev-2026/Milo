...
//        setStatusBarColor();
    ...
    /**
     * 切换状态栏模式
     * 使用 WindowInsetsCompat 方式，正确适配灵动岛/刘海屏
     */
...
//    }
    ...
    private void initTitleBar() {
        ...
        // 使用 WindowInsets 动态设置状态栏占位高度，正确适配灵动岛/刘海屏
        if (!hideStatusBar && statusBarView.getVisibility() == View.VISIBLE) {
            View rootView = getWindow().getDecorView().findViewById(android.R.id.content);
            if (rootView != null) {
                ViewCompat.setOnApplyWindowInsetsListener(rootView, (v, insets) -> {
                    int statusBarHeight = insets.getInsets(WindowInsetsCompat.Type.statusBars()).top;
                    if (statusBarHeight > 0) {
                        ViewGroup.LayoutParams statusBarParams = statusBarView.getLayoutParams();
                        if (statusBarParams != null && statusBarParams.height != statusBarHeight) {
