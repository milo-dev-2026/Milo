...
public class ChatAdapter extends BaseProviderMultiAdapter<WKUIChatMsgItemEntity> {
    ...
    protected int getItemType(@NotNull List<? extends WKUIChatMsgItemEntity> list, int i) {
        ...
        android.util.Log.d("ChatAdapter", "getItemType: msg type=" + list.get(i).wkMsg.type + " not found in provider list, provider keys=" + getItemProviderList().keySet());
    ...
    public long getLastTimeMsg() {
        ...
        for (int i = getData().size() - 1; i >= 0; i--) {
            if (getData().get(i).wkMsg != null && getData().get(i).wkMsg.timestamp > 0
                    && !WKContentType.isLocalMsg(getData().get(i).wkMsg.type)) {
    ...
    //通过消息ID判断是否存在
    public boolean isExistByMessageId(String messageID) {
        if (TextUtils.isEmpty(messageID)) return false;
        for (int i = 0, size = getData().size(); i < size; i++) {
            if (getData().get(i).wkMsg != null
                    && !TextUtils.isEmpty(getData().get(i).wkMsg.messageID)
                    && getData().get(i).wkMsg.messageID.equals(messageID)) {
                return true;
            }
        }
        return false;
    ...
    //通过orderSeq判断是否存在
    public boolean isExistByOrderSeq(long orderSeq) {
        if (orderSeq <= 0) return false;
        for (int i = 0, size = getData().size(); i < size; i++) {
            if (getData().get(i).wkMsg != null && getData().get(i).wkMsg.orderSeq == orderSeq) {
                return true;
            }
        }
        return false;
