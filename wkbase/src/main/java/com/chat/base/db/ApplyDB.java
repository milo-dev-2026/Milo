...
public class ApplyDB {
    ...
    private NewFriendEntity serializeFriend(Cursor cursor) {
        ...
        // 兼容旧版本数据库，type字段可能不存在
        try {
            friendEntity.type = cursor.getInt(cursor.getColumnIndex("type"));
        } catch (Exception e) {
            friendEntity.type = 0;
        }
        try {
            friendEntity.group_no = cursor.getString(cursor.getColumnIndex("group_no"));
        } catch (Exception e) {
            friendEntity.group_no = "";
        }
        try {
            friendEntity.group_name = cursor.getString(cursor.getColumnIndex("group_name"));
        } catch (Exception e) {
            friendEntity.group_name = "";
        }
        return friendEntity;
    }
    ...
    private ContentValues getContentValues(NewFriendEntity friendEntity) {
        ...
        contentValues.put("type", friendEntity.type);
        contentValues.put("group_no", friendEntity.group_no);
        contentValues.put("group_name", friendEntity.group_name);
