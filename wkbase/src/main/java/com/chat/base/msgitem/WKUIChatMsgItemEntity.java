...
public class WKUIChatMsgItemEntity {
    ...
    public WKUIChatMsgItemEntity(IConversationContext conversationContext, WKMsg wkMsg, ILinkClick iLinkClick) {
        ...
        if (wkMsg != null) {
            try {
                ...
                android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans exception: " + e.getMessage(), e);
                // Ensure displaySpans is at least populated with raw content
                if (displaySpans == null && wkMsg.baseContentMsgModel != null) {
                    try {
                        displaySpans = new SpannableStringBuilder();
                        String content = wkMsg.baseContentMsgModel.getDisplayContent();
                        if (content != null) {
    ...
    public void formatSpans(IConversationContext conversationContext, WKMsg wkMsg) {
        ...
        boolean isSend = !TextUtils.isEmpty(wkMsg.fromUID)
                && wkMsg.fromUID.equals(com.chat.base.config.WKConfig.getInstance().getUid());
        int mentionColor = isSend ? android.graphics.Color.WHITE : Theme.colorAccount;

        displaySpans = new SpannableStringBuilder();
        String rawContent = getContent();
        if (rawContent == null) rawContent = "";
        displaySpans.append(rawContent);
        ...
        if (WKReader.isNotEmpty(wkMsg.baseContentMsgModel.entities)) {
            try {
                for (WKMsgEntity entity : wkMsg.baseContentMsgModel.entities) {
                    if ((entity.offset + entity.length) > displaySpans.length() || entity.offset > displaySpans.length() || entity.offset < 0)
                        continue;

                    if (entity.type.equals(ChatContentSpanType.link)) {
                    ...
                    } else if (entity.type.equals(ChatContentSpanType.botCommand)) {
                        displaySpans.setSpan(new UnderlineSpan(), entity.offset, (entity.offset + entity.length), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    }
                }
            } catch (Exception e) {
                android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing link entities", e);
            }
        } else {
            if (wkMsg.baseContentMsgModel.mentionInfo != null) {
                try {
                    ...
                    android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing mentionInfo", e);
                }
            }
            // 默认数据
            try {
                ...
                android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing URLs", e);
            }
            if (wkMsg.baseContentMsgModel.mentionAll == 1) {
                try {
                    ...
                    if (index1 >= 0) {
                        displaySpans.setSpan(new ForegroundColorSpan(mentionColor), index1, (index1 + mentionAll.length()), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                        displaySpans.setSpan(new StyleSpan(Typeface.BOLD), index1, (index1 + mentionAll.length()), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    }
                } catch (Exception e) {
                    android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing mentionAll", e);
                }
            }
        }
        // emoji
        try {
            ...
            android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing emojis", e);
        }
        // 单独处理@效果
        if (WKReader.isNotEmpty(wkMsg.baseContentMsgModel.entities)) {
            try {
                int allOffset = 0;
                for (WKMsgEntity entity : wkMsg.baseContentMsgModel.entities) {
                    if (entity.type.equals(ChatContentSpanType.mention)) {
                        ...
                        // Bounds check using adjusted offsets
                        if (start < 0 || end > displaySpans.length() || start >= end) {
                            continue;
                        }
                ...
                android.util.Log.e("WKUIChatMsgItemEntity", "formatSpans: error processing mention entities", e);
            }
        }
