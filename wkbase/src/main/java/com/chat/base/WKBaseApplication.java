...
public class WKBaseApplication {
    ...
    public String httpPackageName; // HTTP请求头中的包名
    ...
    public void init(@NonNull String packageName, Application context) {
        ...
        this.httpPackageName = packageName; // 默认使用实际包名
...
//            HttpsUtils.SSLParams sslParams1 = HttpsUtils.getSslSocketFactory();
    ...
    private void initCacheDir() {
        WKConstants.avatarCacheDir = getCachePath("wkAvatars");
        WKFileUtils.getInstance().createFileDir(WKConstants.avatarCacheDir);
        WKConstants.imageDir = getCachePath("wkImages");
        WKFileUtils.getInstance().createFileDir(WKConstants.imageDir);
        WKConstants.videoDir = getCachePath("wkVideos");
        WKFileUtils.getInstance().createFileDir(WKConstants.videoDir);
        WKConstants.voiceDir = getCachePath("wkVoices");
        WKFileUtils.getInstance().createFileDir(WKConstants.voiceDir);
        WKConstants.chatBgCacheDir = getCachePath("wkChatBg");
        WKFileUtils.getInstance().createFileDir(WKConstants.chatBgCacheDir);
        WKConstants.messageBackupDir = getCachePath("messageBackup");
        WKFileUtils.getInstance().createFileDir(WKConstants.messageBackupDir);
        WKConstants.chatDownloadFileDir = getCachePath("chatDownloadFile");
    }

    private String getCachePath(String dirName) {
        java.io.File dir = getContext().getExternalFilesDir(dirName);
        if (dir == null) {
            dir = new java.io.File(getContext().getFilesDir(), dirName);
        }
