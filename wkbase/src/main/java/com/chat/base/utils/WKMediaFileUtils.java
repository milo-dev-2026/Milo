...
public class WKMediaFileUtils {
    ...
    //一次性获取视频封面和时长，避免重复创建MediaMetadataRetriever
    public VideoMeta getVideoMeta(String videoPath) {
        MediaMetadataRetriever retriever = new MediaMetadataRetriever();
        try {
            retriever.setDataSource(videoPath);
            String duration = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
            ...
            Bitmap bitmap = retriever.getFrameAtTime();
            String coverPath = WKFileUtils.getInstance().saveBitmap(null, bitmap);
            long seconds = duration != null ? Long.valueOf(duration) / 1000 : 0;
            int width = widthStr != null ? Integer.valueOf(widthStr) : 0;
            int height = heightStr != null ? Integer.valueOf(heightStr) : 0;
            return new VideoMeta(coverPath, seconds, width, height);
        } finally {
            try {
                retriever.release();
            } catch (Exception ignored) {
    ...
    public static class VideoMeta {
        ...
        public VideoMeta(String coverPath, long seconds, int width, int height) {
