...
//        val dir = Environment.getExternalStoragePublicDirectory(type)
    ...
    private fun calculate(mTask: Task) {
        ...
        val bufferedOut = BufferedOutputStream(mTask.fileOutputStream!!, 1024 * 256)
//        try {
...
//        }
        ...
        try {
            while (true) {
                ...
                if (now - lastProgressUpdate > 200 || mTask.hasDownloadSize >= mTask.contentSize) {
                    lastProgressUpdate = now
                    val progress = if (mTask.contentSize > 0) {
                        ((mTask.hasDownloadSize * 1f / mTask.contentSize) * 100).toInt()
                    } else 0
                    mainHandler.post {
                        callback?.first?.invoke(mTask.url, progress)
                    }
                }
            }
        } catch (e: Exception) {
