...
public class WKScanActivity extends BarcodeCameraScanActivity {
    ...
    public void initUI() {
        ...
        ivAlbum = findViewById(R.id.photoAlbumIv);

        // 动态设置返回按钮和右侧按钮的顶部边距，适配灵动岛/刘海屏
        int statusBarHeight = WKStatusBarUtils.getStatusBarHeight(this);
        int marginTop = statusBarHeight + 12; // 状态栏高度 + 12dp 间距
        AppCompatImageView backIv = findViewById(R.id.backIv);
        AppCompatImageView rightIV = findViewById(R.id.rightIV);
        android.view.ViewGroup.MarginLayoutParams backParams = (android.view.ViewGroup.MarginLayoutParams) backIv.getLayoutParams();
        backParams.topMargin = marginTop;
        backIv.setLayoutParams(backParams);
        android.view.ViewGroup.MarginLayoutParams rightParams = (android.view.ViewGroup.MarginLayoutParams) rightIV.getLayoutParams();
        rightParams.topMargin = marginTop;
        rightIV.setLayoutParams(rightParams);
    ...
    protected void onResume() {
        super.onResume();
        // 确保按钮点击事件正确，防止父类在生命周期中覆盖设置
        if (ivFlash != null) {
            ivFlash.setOnClickListener(v -> toggleTorchState());
        }
        if (ivAlbum != null) {
            ivAlbum.setOnClickListener(v -> chooseIMG());
        }
    }
