...
public class FriendModel extends WKBaseModel {
     ...
     * 搜索用户
     */
    public void searchUser(String keyword, int page, int pageSize, final ISearchUserListener listener) {
        request(createAuthService(FriendService.class).searchUser(keyword, page, pageSize), new IRequestResultListener<SearchUserResult>() {
            @Override
            public void onSuccess(SearchUserResult result) {
                if (result != null && result.list != null) {
                    listener.onResult(HttpResponseCode.success, "", result.list, result.total);
                } else {
                    listener.onResult(HttpResponseCode.success, "", new ArrayList<>(), 0);
                }
            }

            @Override
            public void onFail(int code, String msg) {
                listener.onResult(code, msg, null, 0);
            }
        });
    }

    public interface ISearchUserListener {
        void onResult(int code, String msg, List<SearchUserResult.UserItem> list, int total);
    }
}
