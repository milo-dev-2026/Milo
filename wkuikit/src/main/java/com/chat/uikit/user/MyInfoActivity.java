...
public class MyInfoActivity extends WKBaseActivity<ActMyInfoLayoutBinding> {
    ...
    protected void initView() {
        ...
        String localSig = WKSharedPreferencesUtil.getInstance().getSPWithUID("user_signature");
        if (!TextUtils.isEmpty(localSig)) {
            wkVBinding.tvPersonalSignature.setText(localSig);
        }
    ...
    protected void initListener() {
        WKCommonModel.getInstance().getChannel(WKConfig.getInstance().getUid(), WKChannelType.PERSONAL, (code, msg, entity) -> {
            if (entity != null && entity.extra != null) {
                ...
                Object signatureObject = entity.extra.get("signature");
                if (signatureObject != null) {
                    String signature = (String) signatureObject;
                    if (!TextUtils.isEmpty(signature)) {
                        wkVBinding.tvPersonalSignature.setText(signature);
                    }
                }
            }
        });
        ...
        SingleClickUtil.onSingleClick(wkVBinding.llPersonalSignature, view1 -> {
            Intent intent = new Intent(this, PersonalSignatureActivity.class);
            intent.putExtra("channelId", WKConfig.getInstance().getUid());
            intent.putExtra("oldStr", wkVBinding.tvPersonalSignature.getText().toString());
            chooseResultLac.launch(intent);
        });
    ...
    ActivityResultLauncher<Intent> chooseResultLac = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
        if (result.getResultCode() == RESULT_OK && result.getData() != null) {
            ...
            if (updateType == 1) {
                ...
                wkVBinding.tvPersonalSignature.setText(resultStr);
            }
        }
    });
}
