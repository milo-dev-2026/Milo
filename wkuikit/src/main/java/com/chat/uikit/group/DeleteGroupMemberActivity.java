...
public class DeleteGroupMemberActivity extends WKBaseActivity<ActChooseContactsLayoutBinding> implements OnQuickSideBarTouchListener {
    ...
    protected void rightLayoutClick() {
        ...
        if (groupMemberAdapter != null && groupMemberAdapter.getData() != null) {
            for (int i = 0, size = groupMemberAdapter.getData().size(); i < size; i++) {
                GroupMemberEntity item = groupMemberAdapter.getData().get(i);
                if (item != null && item.checked == 1)
        ...
        if (WKReader.isNotEmpty(selectedList)) {
            ...
            for (int i = 0, size = selectedList.size(); i < size; i++) {
                GroupMemberEntity item = selectedList.get(i);
                if (item != null && item.member != null) {
                    uids.add(item.member.memberUID);
                    names.add(item.member.memberName);
                }
            }
    ...
    protected void initView() {
        ...
        selectedAdapter = new ChooseUserSelectedAdapter(this, new ChooseUserSelectedAdapter.IGetEdit() {
            @Override
    ...
    protected void initData() {
        ...
        if (WKReader.isEmpty(channelMembers)) {
            // 本地没有成员数据，从服务器拉取
            loadingPopup.show();
            fetchMembersFromServer();
        } else {
            refreshMemberList(channelMembers);
        }
    }
     ...
     * 从服务器拉取群成员列表
     */
    private void fetchMembersFromServer() {
        WKIM.getInstance().getChannelMembersManager().getWithPageOrSearch(groupId, WKChannelType.GROUP, "", 1, 100, (list, b) -> {
     ...
     * 处理并刷新成员列表
     */
    private void refreshMemberList(List<WKChannelMember> channelMembers) {
        String loginUID = WKConfig.getInstance().getUid();
        List<GroupMemberEntity> list = new ArrayList<>();
        int loginMemberRole = WKChannelMemberRole.normal;
        // 从成员列表中查找当前用户的角色，比单独调用getMember更可靠
        if (channelMembers != null) {
            for (WKChannelMember member : channelMembers) {
                if (member.memberUID.equals(loginUID)) {
                    loginMemberRole = member.role;
                    break;
                }
            }
        }
        // 备份：如果列表中没找到，再尝试从管理器获取
        if (loginMemberRole == WKChannelMemberRole.normal) {
        ...
        if (channelMembers != null) {
            for (int i = 0, size = channelMembers.size(); i < size; i++) {
                boolean isAdd = (channelMembers.get(i).role == WKChannelMemberRole.manager && loginMemberRole == WKChannelMemberRole.admin)
                        || channelMembers.get(i).role == WKChannelMemberRole.normal;
                if (isAdd) {
    ...
    private void searchUser(String content) {
        ...
        for (int i = 0, size = allList.size(); i < size; i++) {
            GroupMemberEntity item = allList.get(i);
            if (item == null || item.member == null) continue;
            boolean match = false;
            if (!TextUtils.isEmpty(item.member.memberName)
                    && item.member.memberName.toLowerCase(Locale.getDefault()).contains(lowerContent)) {
                match = true;
            }
            if (!match && !TextUtils.isEmpty(item.member.memberRemark)
                    && item.member.memberRemark.toLowerCase(Locale.getDefault()).contains(lowerContent)) {
                match = true;
            }
            if (!match && !TextUtils.isEmpty(item.member.remark)
                    && item.member.remark.toLowerCase(Locale.getDefault()).contains(lowerContent)) {
                match = true;
            }
            if (!match) {
                String pinyin = item.pying != null ? item.pying : "#";
                if (lowerContent.contains(pinyin.toLowerCase(Locale.getDefault()))) {
                    match = true;
                }
            }
            if (match) {
    ...
    public void onLetterChanged(String letter, int position, float y) {
        ...
        if (WKReader.isNotEmpty(list)) {
            for (int i = 0, size = list.size(); i < size; i++) {
                if (list.get(i).pying != null && list.get(i).pying.startsWith(letter)) {
