...
public class GroupDetailActivity extends WKBaseActivity<ActGroupDetailLayoutBinding> implements GroupContract.GroupView {
    ...
    private static final int REQUEST_CODE_PINNED_MSG = 1002;
    private ActivityResultLauncher<Intent> cropAvatarLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
        if (result.getResultCode() == RESULT_OK && result.getData() != null) {
            String path = result.getData().getStringExtra("path");
            if (TextUtils.isEmpty(path)) return;
            uploadGroupAvatar(path);
        }
    });
    ...
    protected void initListener() {
        ...
        SingleClickUtil.onSingleClick(wkVBinding.pinnedMsgLayout, view1 -> {
            if (groupIsEnable()) {
                Intent intent = new Intent(GroupDetailActivity.this, com.chat.uikit.chat.pin.MessagePinActivity.class);
                intent.putExtra("channelId", groupNo);
                intent.putExtra("channelType", WKChannelType.GROUP);
                startActivityForResult(intent, REQUEST_CODE_PINNED_MSG);
            }
        });
        ...
        // 解散群聊
        SingleClickUtil.onSingleClick(wkVBinding.disbandGroupLayout, v -> {
            if (!groupIsEnable()) return;
            WKDialogUtils.getInstance().showDialog(this,
                    getString(R.string.disband_group_chat),
                    getString(R.string.disband_group_tips),
                    true, "", getString(R.string.disband_group_chat), 0,
                    ContextCompat.getColor(this, R.color.red), index -> {
                        if (index == 1) {
                            GroupModel.getInstance().disbandGroup(groupNo, (code, msg) -> {
                                if (code == HttpResponseCode.success) {
                                    WKIM.getInstance().getMsgManager().clearWithChannel(groupNo, WKChannelType.GROUP);
                                    MsgModel.getInstance().offsetMsg(groupNo, WKChannelType.GROUP, null);
                                    WKIM.getInstance().getConversationManager().deleteWitchChannel(groupNo, WKChannelType.GROUP);
        ...
        SingleClickUtil.onSingleClick(wkVBinding.groupAdminLayout, view1 -> {
            if (groupIsEnable()) {
                Intent intent = new Intent(this, GroupManageActivity.class);
                intent.putExtra("groupNo", groupNo);
                intent.putExtra("scroll_to", "admin");
                startActivity(intent);
            }
        });
        SingleClickUtil.onSingleClick(wkVBinding.groupApproveLayout, view1 -> {
            if (groupIsEnable()) {
                Intent intent = new Intent(this, GroupManageActivity.class);
                intent.putExtra("groupNo", groupNo);
                intent.putExtra("scroll_to", "top");
                startActivity(intent);
            }
        });
        ...
        SingleClickUtil.onSingleClick(wkVBinding.noticeLayout, view1 -> {
            ...
            if (groupChannel.localExtra != null && groupChannel.localExtra.containsKey(WKChannelExtras.notice)) {
                Object noticeObj = groupChannel.localExtra.get(WKChannelExtras.notice);
                if (noticeObj instanceof String) {
                    notice = (String) noticeObj;
                }
            }
        ...
        // 群头像点击 - 管理员/群主可更换头像，普通成员仅查看大图
        SingleClickUtil.onSingleClick(wkVBinding.groupAvatarLayout, v -> {
            if (!groupIsEnable()) return;
            if (groupChannel != null) {
                if (memberRole == WKChannelMemberRole.admin || memberRole == WKChannelMemberRole.manager) {
                    // 管理员/群主：弹出选择菜单
                    showAvatarOptions();
                } else {
                    // 普通成员：仅查看大图
        ...
        WKIM.getInstance().getChannelMembersManager().addOnAddChannelMemberListener("group_detail_add_channel_member", list -> {
            ...
            if (WKReader.isNotEmpty(list)) {
                ...
                if (groupType == WKGroupType.normalGroup) {
                    ...
                    if (memberRole != WKChannelMemberRole.normal) {
                        int insertPos = Math.max(0, groupMemberAdapter.getData().size() - 2);
                        groupMemberAdapter.addData(insertPos, tempList);
                    } else {
                        int insertPos = Math.max(0, groupMemberAdapter.getData().size() - 1);
                        groupMemberAdapter.addData(insertPos, tempList);
                    }
    ...
    private void resortData(List<WKChannelMember> list) {
        ...
        // 从成员列表中查找当前用户的角色，比单独调用getMember更可靠
        if (list != null) {
            for (WKChannelMember member : list) {
                if (member.memberUID.equals(loginUID)) {
        ...
        // 备份：如果列表中没找到，再尝试从管理器获取
        if (memberRole == WKChannelMemberRole.normal) {
            WKChannelMember channelMember = WKIM.getInstance().getChannelMembersManager().getMember(groupNo, WKChannelType.GROUP, loginUID);
            if (channelMember != null && channelMember.memberUID.equals(loginUID)) {
        ...
        if (list != null) {
            ...
            // 群主显示解散群聊入口
            if (memberRole == WKChannelMemberRole.admin) {
                wkVBinding.disbandGroupLayout.setVisibility(View.VISIBLE);
            } else {
                wkVBinding.disbandGroupLayout.setVisibility(View.GONE);
            }
            // 普通成员隐藏群管理入口（群管理页面）
            if (memberRole == WKChannelMemberRole.normal) {
                wkVBinding.groupManageLayout.setVisibility(View.GONE);
    ...
    public void onRefreshGroupSetting(String key, int value) {
        // 群设置更新后，更新本地 WKIM 频道缓存并刷新 UI
        groupChannel = WKIM.getInstance().getChannelManager().getChannel(groupNo, WKChannelType.GROUP);
        if (groupChannel != null) {
            // 更新本地缓存中的用户级设置字段，确保UI立即反映变化
            boolean needSave = false;
            switch (key) {
                case "save":
                    groupChannel.save = value;
                    needSave = true;
                    break;
                case "mute":
                    groupChannel.mute = value;
                    needSave = true;
                    break;
                case "top":
                    groupChannel.top = value;
                    needSave = true;
                    break;
                case "show_nick":
                    groupChannel.showNick = value;
                    needSave = true;
                    break;
                case "receipt":
                    groupChannel.receipt = value;
                    needSave = true;
                    break;
                case "flame":
                    groupChannel.flame = value;
                    needSave = true;
                    break;
            }
            if (needSave) {
                WKIM.getInstance().getChannelManager().saveOrUpdateChannel(groupChannel);
            }
            setData();
        }
        // 显示操作成功反馈
        ...
        switch (key) {
            case "mute":
                toastMsg = value == 1 ? "已开启消息免打扰" : "已关闭消息免打扰";
                break;
            case "top":
                toastMsg = value == 1 ? "已置顶聊天" : "已取消置顶聊天";
                break;
            case "save":
                toastMsg = value == 1 ? "已保存到通讯录" : "已从通讯录移除";
                break;
            case "show_nick":
                toastMsg = value == 1 ? "已开启群昵称显示" : "已关闭群昵称显示";
                break;
            case "receipt":
                toastMsg = value == 1 ? "已开启消息回执" : "已关闭消息回执";
                break;
        }
        if (toastMsg != null) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        } else if (requestCode == REQUEST_CODE_PINNED_MSG && resultCode == RESULT_OK) {
            // 置顶消息被点击，刷新置顶状态
            initData();
        }
    }
    ...
    private void setNotice() {
        ...
        if (hashMap != null) {
            if (hashMap.containsKey(WKChannelExtras.notice)) {
                Object noticeObj = hashMap.get(WKChannelExtras.notice);
                if (noticeObj instanceof String) {
                    notice = (String) noticeObj;
                }
            }
    ...
    private void setData() {
        ...
        // 设置群头像缩略图
        wkVBinding.groupAvatarIv.showAvatar(groupNo, WKChannelType.GROUP, groupChannel.avatarCacheKey);
    ...
    private void showAvatarOptions() {
        ...
        list.add(new BottomSheetItem(getString(R.string.preview), R.mipmap.msg_gallery, () -> previewGroupAvatar()));
        list.add(new BottomSheetItem(getString(R.string.update_avatar), R.mipmap.msg_edit, () -> chooseAvatarIMG()));
        WKDialogUtils.getInstance().showBottomSheet(this, null, true, list);
    }

    /**
     * 预览群头像大图
     */
    private void previewGroupAvatar() {
        if (groupChannel != null) {
            Object[] urls = new Object[]{groupChannel.avatarCacheKey != null ? groupChannel.avatarCacheKey : ""};
            EndpointManager.getInstance().invoke("preview_image", urls);
        }
    }
     ...
     * 打开图片选择器，选择后跳转裁剪
     */
    private void openAvatarImagePicker() {
    ...
    private void uploadGroupAvatar(String path) {
        GroupModel.getInstance().uploadGroupAvatar(groupNo, path, code -> {
            if (code == HttpResponseCode.success) {
                if (groupChannel != null) {
                    groupChannel.avatarCacheKey = UUID.randomUUID().toString().replace("-", "");
                    WKIM.getInstance().getChannelManager().updateAvatarCacheKey(groupNo, WKChannelType.GROUP, groupChannel.avatarCacheKey);
                    wkVBinding.groupAvatarIv.showAvatar(groupNo, WKChannelType.GROUP, groupChannel.avatarCacheKey);
                }
                showToast(getString(com.chat.base.R.string.avatar_upload_success));
            } else {
                showToast(getString(com.chat.base.R.string.avatar_upload_fail));
            }
        });
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        EndpointManager.getInstance().remove("group_detail");
