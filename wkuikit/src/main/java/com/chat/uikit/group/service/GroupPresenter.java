...
public class GroupPresenter implements GroupContract.GroupPresenter {
    ...
    public void updateGroupSetting(String groupNo, String key, int value) {
        // 用户级设置（save/mute/top/show_nick等）走 mysetting 接口
        if (GroupModel.getInstance().isMySettingKey(key)) {
            GroupModel.getInstance().updateMyGroupSetting(groupNo, key, value, (code, msg) -> {
                if (code == HttpResponseCode.success) {
