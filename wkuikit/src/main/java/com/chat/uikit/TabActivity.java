...
public class TabActivity extends WKBaseActivity<ActTabMainBinding> {
    ...
    private final boolean isShowTabText = false;
...
//        wkVBinding.vp.setUserInputEnabled(false);
        ...
        // 通知权限只在首次启动时请求一次，避免每次进入都重复请求
        checkNotificationPermissionOnce();
...
//        workplaceIV = new RLottieImageView(this);
        ...
        Typeface face;
        try {
            face = Typeface.createFromAsset(getResources().getAssets(), "fonts/mw_bold.ttf");
...
//            }
    ...
    private boolean hasNotificationPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            return androidx.core.content.ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) == android.content.pm.PackageManager.PERMISSION_GRANTED;
        } else {
    ...
    @SuppressLint("CheckResult")
    private void checkNotificationPermissionOnce() {
        // 如果已经授权，不做任何事
        if (hasNotificationPermission()) return;
        // 如果已经请求过（不管用户同意还是拒绝），不再重复请求系统权限弹窗
        boolean alreadyRequested = WKSharedPreferencesUtil.getInstance().getBoolean(KEY_NOTIFICATION_PERMISSION_REQUESTED);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            rxPermissions.request(Manifest.permission.POST_NOTIFICATIONS).subscribe(aBoolean -> {
                WKSharedPreferencesUtil.getInstance().putBoolean(KEY_NOTIFICATION_PERMISSION_REQUESTED, true);
                if (!aBoolean) {
    ...
    public void setMsgCount(int number) {
        android.util.Log.d("WKBadgeDebug", "setMsgCount: number=" + number);
        WKUIKitApplication.getInstance().setTotalMsgCount(number);
        if (number > 0) {
