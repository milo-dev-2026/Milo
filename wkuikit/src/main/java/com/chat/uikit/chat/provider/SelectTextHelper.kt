...
//            return false
    ...
    fun dismissCursorHandles() {
        mStartHandle?.dismiss()
        mEndHandle?.dismiss()
    }

    fun clearSelectionBackground() {
        android.util.Log.e("SelectTextHelper", "clearSelectionBackground: mSpan=$mSpan, mSpannable=$mSpannable")
        if (mSpannable != null && mSpan != null) {
            mSpannable!!.removeSpan(mSpan)
            mSpan = null
            android.util.Log.e("SelectTextHelper", "clearSelectionBackground: removed BackgroundColorSpan")
        }
...
//        } // 此setMovementMethod可被修改
    ...
    private fun showAllView() {
        android.util.Log.e("SelectTextHelper", "showAllView called")
        reset()
        isHide = false
        if (mStartHandle == null) mStartHandle = CursorHandle(true)
        if (mEndHandle == null) mEndHandle = CursorHandle(false)
        ...
        if (mSpannable == null) {
            android.util.Log.e("SelectTextHelper", "showAllView: mSpannable is null, returning")
            return
        }
        android.util.Log.e("SelectTextHelper", "showAllView: calling selectText(0, ${mTextView.text.length})")
        selectText(0, mTextView.text.length)
        android.util.Log.e("SelectTextHelper", "showAllView: showing cursor handles")
        showCursorHandle(mStartHandle)
        showCursorHandle(mEndHandle)
        android.util.Log.e("SelectTextHelper", "showAllView: calling showOperateWindow")
        showOperateWindow()
    }

    private fun showCursorHandle(cursorHandle: CursorHandle?) {
        // CursorHandle 不再显示，长按菜单由 ChatMessageContextOverlay 处理
        return
    ...
    private fun selectText(startPos: Int, endPos: Int) {
        ...
        if (mSpannable != null) {
            // 不再设置 BackgroundColorSpan，避免蓝色气泡上出现白色涂层
...
//        }
    ...
    private fun resetEmojiBackground() {
        ...
        // 同时清除所有 BackgroundColorSpan
        if (mSpannable != null) {
            val spans = mSpannable!!.getSpans(0, mSpannable!!.length, android.text.style.BackgroundColorSpan::class.java)
            for (span in spans) {
...
//            }
        ...
        fun show() {
            // OperateWindow 不再显示，长按菜单由 ChatMessageContextOverlay 处理
            return
        }
