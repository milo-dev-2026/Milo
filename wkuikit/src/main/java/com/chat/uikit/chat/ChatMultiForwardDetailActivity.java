...
public class ChatMultiForwardDetailActivity extends WKBaseActivity<ActCommonListLayoutWhiteBinding> {
    ...
    protected void setTitle(TextView titleTv) {
        ...
        if (WKMultiForwardContent.channelType == 1) {
            ...
            } else if (!WKMultiForwardContent.userList.isEmpty()) {
                title = WKMultiForwardContent.userList.get(0).channelName;
            } else {
    ...
    protected void initPresenter() {
        ...
        if (!(msg.baseContentMsgModel instanceof WKMultiForwardContent)) {
            showToast("传入数据有误！");
            finish();
            return;
        }
