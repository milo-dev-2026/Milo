...
//            showContent(baseViewHolder, item);
    ...
    private String getContent(WKMsg msg) {
        ...
        // 截图提醒消息是系统提示类，不在会话列表中显示预览内容
        if (msg.type == WKContentType.screenshot) return content;
    ...
    private void setStatus(BaseViewHolder helper, WKUIConversationMsg item, boolean isPlayAnimation) {
        RLottieImageView sendingMsgIv = helper.itemView.findViewById(R.id.statusIV);
        if (sendingMsgIv == null) {
            return;
        }
    ...
    private void setUnreadCount(@NotNull BaseViewHolder baseViewHolder, ChatConversationMsg item, boolean isAnimated) {
        ...
        int badgeColor = isMute ? R.color.color999 : R.color.reminderColor;

        counterView.setColors(R.color.white, badgeColor);
        ...
        if (item.getWkChannel() != null) {
            if (item.channelType == WKChannelType.COMMUNITY) {
                ...
                if (item.channelType == WKChannelType.PERSONAL) {
                    avatarView.setOnline(item.getWkChannel().online == 1);
                } else {
...
//                if (!isScrolling)
            ...
            if (item.getWkChannel().robot == 1)
                categoryLayout.addView(Theme.getChannelCategoryTV(getContext(), getContext().getString(R.string.bot), ContextCompat.getColor(getContext(), R.color.color_main), ContextCompat.getColor(getContext(), R.color.white), ContextCompat.getColor(getContext(), R.color.color_main)), LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER, 5, 1, 0, 0));
            //判断是否禁言
            if (WKChannelUtils.getInstance().isGroupForbidden(item.getWkChannel())) {
