...
//        )
        ...
        if (from == WKChatIteMsgFromType.SEND) {
            contentTv?.setBackgroundResource(0) // 移除 TextView 背景，统一用 BubbleLayout
        ...
        // Fallback: if displaySpans is null or empty, use raw content from the message model
        val displayText: CharSequence? = if (uiChatMsgItemEntity.displaySpans != null
            && uiChatMsgItemEntity.displaySpans.isNotEmpty()
        ) {
            uiChatMsgItemEntity.displaySpans
        } else {
            val baseModel = uiChatMsgItemEntity.wkMsg?.baseContentMsgModel
            val rawContent = baseModel?.displayContent
            if (!rawContent.isNullOrEmpty()) {
                android.util.Log.w("WKTextProvider", "setData: displaySpans was null/empty, using raw content as fallback")
                rawContent
            } else {
                uiChatMsgItemEntity.displaySpans
            }
...
//        textMsgBean = msgBean
        ...
        val builder = SelectTextHelper.Builder(textView, fullLayout) // 放你的textView到这里！！
            ...
            .setSelectedColor(android.graphics.Color.TRANSPARENT) // 选中文本的颜色-透明，不显示选中背景
...
//                        )
    ...
    private fun showPopup(uiChatMsgItemEntity: WKUIChatMsgItemEntity, v: View, local: FloatArray) {
        android.util.Log.e("WKTextProvider", "=== showPopup START ===")
        mSelectableTextHelper?.dismissCursorHandles()
        mSelectableTextHelper?.clearSelectionBackground()
        mSelectableTextHelper?.dismissOperateWindow()
        android.util.Log.e("WKTextProvider", "showPopup: dismissed cursor+background+operateWindow")
        // 确保在下一帧也清除选中背景（overlay 是异步创建的）
        v.post {
            android.util.Log.e("WKTextProvider", "showPopup: post - clearing again")
            mSelectableTextHelper?.clearSelectionBackground()
            mSelectableTextHelper?.dismissCursorHandles()
        }
...
//        }
    ...
    override fun resetCellBackground(
        ...
        // 气泡宽度自适应文字内容：只设置maxWidth限制最大宽度，不固定宽度
        // 白色气泡和蓝色气泡使用完全相同的宽度自适应逻辑
        val horizontalPadding = contentTvLayout.paddingStart + contentTvLayout.paddingEnd
        val textSizePx = contentTv?.textSize ?: 0f
        val char16Width = (textSizePx * 16).toInt()
        val bubbleMaxWidth = minOf(maxWidth - horizontalPadding, char16Width)
        contentTv?.maxWidth = bubbleMaxWidth
        // 让气泡wrap_content，根据文字内容自适应宽度
        textContentLayout.layoutParams.width = ViewGroup.LayoutParams.WRAP_CONTENT
        // 确保 BubbleLayout 也是 wrap_content，不固定宽度
        val bubbleLp = contentTvLayout.layoutParams
        if (bubbleLp != null && bubbleLp.width != ViewGroup.LayoutParams.WRAP_CONTENT) {
            bubbleLp.width = ViewGroup.LayoutParams.WRAP_CONTENT
            contentTvLayout.layoutParams = bubbleLp
        }
