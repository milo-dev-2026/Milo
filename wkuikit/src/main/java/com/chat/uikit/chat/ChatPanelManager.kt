...
class ChatPanelManager(
    ...
    private val panelView: LinearLayout = parentView.findViewById(R.id.panelView)
    ...
    fun chatAvatarClick(uid: String, isLongClick: Boolean) {
        if (isLongClick) {
            ...
            if (iConversationContext.chatChannelInfo.channelType == WKChannelType.GROUP) {
                ...
                if (loginMember != null) {
                    if ((isGroupForbidden(iConversationContext.chatChannelInfo) && loginMember.role == WKChannelMemberRole.normal) || loginMember.forbiddenExpirationTime > 0) {
                        return
                    }
                }
                val member =
    ...
    private fun initTool() {
        ...
        toolBarAdapter?.setOnItemChildClickListener { adapter1: BaseQuickAdapter<*, *>, view: View, position: Int ->
            if (view.id == R.id.imageView) {
                android.util.Log.d("PanelDebug", "Tool bar item clicked, position=$position")
                SingleClickUtil.determineTriggerSingleClick(view, 500) {
                    ...
                    android.util.Log.d("PanelDebug", "Menu sid=${mChatToolBarMenu.sid}, bottomView=${mChatToolBarMenu.bottomView != null}, isSelected=${mChatToolBarMenu.isSelected}")
...
//        }
        ...
        parentView.post {
            ...
            val availableHeight = if (parentView.top > 0) parentView.top else followScrollLayout.height
            remindHeaderView!!.layoutParams.height =
                availableHeight - AndroidUtilities.dp(
                    min(
...
//            }
        ...
        remindMemberAdapter!!.setOnItemClickListener { adapter, _, position ->
            val entity = adapter.getItem(position) as GroupMemberEntity?
            if (entity != null) {
    ...
    private fun initRobotGIF() {
        ...
        robotGIFAdapter!!.setOnItemClickListener { adapter, _, position ->
            val entity = adapter.getItem(position) as WKRobotGIFEntity
...
//            CommonAnim.getInstance().showOrHide(closeSearchLottieIV, false, true)
    ...
    private fun initListener() {
        ...
        // 点击输入框时，如果面板已打开，则切换到键盘状态（确保面板与键盘互斥）
        ...
        sendIV.setOnClickListener {
            ...
            if (!TextUtils.isEmpty(content)) {
                ...
                textMsgModel.entities = editText.allEntity.map { entity ->
                    WKMsgEntity().apply {
...
//            val keyword = mentionEnd(content)
            ...
            val availableHeight = if (parentView.top > 0) parentView.top else followScrollLayout.height
            remindHeaderView!!.layoutParams.height =
                availableHeight - AndroidUtilities.dp((min * height))
            remindRecycleView!!.setHeaderViewY(remindHeaderView!!.layoutParams.height.toFloat())
            if (remindRecycleView!!.visibility != View.VISIBLE || remindRecycleView!!.animation != null) {
                remindRecycleView!!.clearAnimation()
                remindRecycleView!!.visibility = View.VISIBLE
                remindRecycleView!!.post {
                    val postMin =
                        (remindMemberAdapter!!.itemCount - remindMemberAdapter!!.headerLayoutCount).coerceAtMost(3)
                    var postHeight = 40f
                    if (remindMemberAdapter!!.data.size > 3) postHeight = 48f
                    val postAvailableHeight = if (parentView.top > 0) parentView.top else followScrollLayout.height
                    remindHeaderView!!.layoutParams.height =
                        postAvailableHeight - AndroidUtilities.dp((postMin * postHeight))
                    remindRecycleView!!.setHeaderViewY(remindHeaderView!!.layoutParams.height.toFloat())
                }
            }
        }
...
//            initRobotGIF(iConversationContext!!)
    ...
    private fun getEmojiLayout(): View {
        ...
        emojiAdapter.setOnItemClickListener { adapter, _, position ->
            val emojiText = adapter.getItem(position) as String
            val curPosition: Int = editText.selectionStart
            val sb = java.lang.StringBuilder(
                Objects.requireNonNull(editText.text).toString()
            )
            sb.insert(curPosition, emojiText)
            MoonUtil.addEmojiSpan(editText, emojiText, iConversationContext.chatActivity)
            editText.setSelection(curPosition + emojiText.length)
        }
        return emojiLayout
    }
    ...
    private fun toolBarClick(
        ...
        android.util.Log.d("PanelDebug", "toolBarClick called, sid=${mChatToolBarMenu.sid}, helper=$helper")
        //存在点击显示的view
        if (mChatToolBarMenu.bottomView != null) {
            if (mChatToolBarMenu.isSelected) {
                ...
                if (helper != null) {
                    if (helper?.isPanelState() == false) {
                        helper?.toPanelState(R.id.emotionView)
                    }
                } else {
                    // 手动显示面板
                    android.util.Log.d("PanelDebug", "Manual show moreLayout, visibility=${moreLayout.visibility}")
                    moreLayout.visibility = View.VISIBLE
                    moreLayout.post {
                        android.util.Log.d("PanelDebug", "After VISIBLE: moreLayout width=${moreLayout.width}, height=${moreLayout.height}, top=${moreLayout.top}, bottom=${moreLayout.bottom}, vis=${moreLayout.visibility}")
                    }
                }
                moreLayout.removeAllViews()
                android.util.Log.d("PanelDebug", "Adding bottomView to moreLayout")
                moreLayout.addView(
