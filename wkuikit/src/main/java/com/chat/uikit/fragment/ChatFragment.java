...
import com.chat.uikit.chat.ConversationSwipeCallback;
...
public class ChatFragment extends WKBaseFragment<FragChatConversationLayoutBinding> {
    ...
    protected void initView() {
        ...
        wkVBinding.titleTv.setTypeface(face);
    ...
    protected void initListener() {
        if (!WKUIKitApplication.getInstance().isIMInited()) {
            Log.e("ChatFragment", "initListener skipped: IM not initialized");
            return;
        }
        ...
        ConversationSwipeCallback swipeCallback = new ConversationSwipeCallback(requireActivity(), new ConversationSwipeCallback.SwipeActions() {
            @Override
            public String getMuteText(int position) {
                ChatConversationMsg item = chatConversationAdapter.getItem(position);
                if (item != null && item.uiConversationMsg != null && item.uiConversationMsg.getWkChannel() != null) {
                    return item.uiConversationMsg.getWkChannel().mute == 1 ? "开启通知" : "关闭通知";
                }
                return "关闭通知";
            }
            ...
            public void onMute(int position) {
                ...
                if (item != null && item.uiConversationMsg != null) {
                    ...
                    if (item.uiConversationMsg.getWkChannel() != null) {
                        mute = item.uiConversationMsg.getWkChannel().mute == 1;
                    }
                    if (item.uiConversationMsg.channelType == WKChannelType.GROUP) {
                        GroupModel.getInstance().updateMyGroupSetting(item.uiConversationMsg.channelID, "mute", mute ? 0 : 1, (code, msg) -> {
                            if (code != HttpResponseCode.success) {
                                WKToastUtils.getInstance().showToastNormal(msg);
                            }
                        });
                    } else {
                        FriendModel.getInstance().updateUserSetting(item.uiConversationMsg.channelID, "mute", mute ? 0 : 1, (code, msg) -> {
                            if (code != HttpResponseCode.success) {
                                WKToastUtils.getInstance().showToastNormal(msg);
                            }
                        });
            ...
            @Override
            public String getPinText(int position) {
                ChatConversationMsg item = chatConversationAdapter.getItem(position);
                if (item != null && item.uiConversationMsg != null && item.uiConversationMsg.getWkChannel() != null) {
                    return item.uiConversationMsg.getWkChannel().top == 1 ? "取消置顶" : "置顶";
                }
                return "置顶";
            }
            ...
            public void onPin(int position) {
                ...
                if (item != null && item.uiConversationMsg != null) {
                    boolean top = false;
                    if (item.uiConversationMsg.getWkChannel() != null) {
                        top = item.uiConversationMsg.getWkChannel().top == 1;
                    }
                    updateTop(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType, top ? 0 : 1);
                }
            }
            ...
            public void onDelete(int position) {
                ...
                if (item != null && item.uiConversationMsg != null) {
                    WKDialogUtils.getInstance().showDialog(getActivity(), getString(R.string.delete_chat), getString(R.string.delete_conver_msg_tips), true, "", getString(R.string.base_delete), 0, ContextCompat.getColor(requireActivity(), R.color.red), index -> {
                        if (index == 1) {
                            List<WKReminder> list = WKIM.getInstance().getReminderManager().getReminders(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType);
                            ...
                            MsgModel.getInstance().offsetMsg(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType, null);
                            WKIM.getInstance().getReminderManager().saveOrUpdateReminders(list);
                            MsgModel.getInstance().clearUnread(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType, 0, null);
                            boolean result = WKIM.getInstance().getConversationManager().deleteWitchChannel(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType);
                            if (result) {
                                if (item.uiConversationMsg.getWkChannel() != null && item.uiConversationMsg.getWkChannel().top == 1) {
                                    updateTop(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType, 0);
                                }
                                WKIM.getInstance().getMsgManager().clearWithChannel(item.uiConversationMsg.channelID, item.uiConversationMsg.channelType);
                            }
                        }
                    });
                }
        ...
        swipeCallback.attachToRecyclerView(wkVBinding.recyclerView);
        chatConversationAdapter.addListener((menu, item) -> {
            if (menu == ChatConversationAdapter.ItemMenu.delete) {
                ...
                if (item.channelType == WKChannelType.GROUP) {
                    GroupModel.getInstance().updateMyGroupSetting(item.channelID, "mute", mute ? 0 : 1, (code, msg) -> {
                        if (code != HttpResponseCode.success) {
                            WKToastUtils.getInstance().showToastNormal(msg);
                        }
                    });
                    ...
                    if (wkCmd.paramJsonObject != null) {
                        ...
                        if (!TextUtils.isEmpty(uid)) {
                            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                                ...
                                if (convMsg != null && convMsg.uiConversationMsg != null
                                        && convMsg.uiConversationMsg.channelType == WKChannelType.PERSONAL
                                        && uid.equals(convMsg.uiConversationMsg.channelID)) {
                                    if (convMsg.uiConversationMsg.getWkChannel() != null) {
                                        convMsg.uiConversationMsg.getWkChannel().online = online;
                                        convMsg.uiConversationMsg.getWkChannel().deviceFlag = device_flag;
                                    }
                                    final int idx = i;
                                    if (getActivity() != null) {
                                        getActivity().runOnUiThread(() -> chatConversationAdapter.notifyItemChanged(idx));
                                    }
                                    break;
                                }
                            }
...
//                        chatConversationAdapter.notifyItemChanged(i, chatConversationAdapter.getData().get(i));
        ...
        WKIM.getInstance().getConnectionManager().addOnConnectionStatusListener("chat_fragment", (i, reason) -> {
            ...
            if (getActivity() != null && !getActivity().isFinishing()) {
                getActivity().runOnUiThread(() -> updateTitleByConnectionStatus(i));
            }
        });
    ...
    protected void initData() {
        if (!WKUIKitApplication.getInstance().isIMInited()) {
            Log.e("ChatFragment", "initData skipped: IM not initialized");
            return;
        }
        getData();
    }
        ...
        try {
            ...
            if (WKReader.isNotEmpty(tempList)) {
                for (int i = 0, size = tempList.size(); i < size; i++) {
                    WKUIConversationMsg convMsg = tempList.get(i);
                    // 过滤掉无频道名且最后一条消息是截图提醒的幽灵会话（防止截图功能产生的异常会话）
                    if (convMsg.getWkMsg() != null
                            && convMsg.getWkMsg().type == WKContentType.screenshot
                            && convMsg.getWkChannel() != null
                            && TextUtils.isEmpty(convMsg.getWkChannel().channelName)
                            && TextUtils.isEmpty(convMsg.getWkChannel().channelRemark)) {
                        continue;
                    }
                    list.add(new ChatConversationMsg(convMsg));
                }
            }
        } catch (Exception e) {
            Log.e("ChatFragment", "getChatMsg error: " + e.getMessage(), e);
        }
        return list;
    }
    ...
    private void setAllCount() {
        ...
        android.util.Log.d("WKBadgeDebug", "setAllCount: allCount=" + allCount + ", tabActivity=" + (tabActivity != null));
        if (tabActivity != null) {
    ...
    private void resetData(WKUIConversationMsg uiConversationMsg, boolean isEnd) {
        ...
        // 过滤掉无频道名且最后一条消息是截图提醒的幽灵会话（防止截图功能产生的异常会话）
        if (uiConversationMsg.getWkMsg() != null
                && uiConversationMsg.getWkMsg().type == WKContentType.screenshot
                && uiConversationMsg.getWkChannel() != null
                && TextUtils.isEmpty(uiConversationMsg.getWkChannel().channelName)
                && TextUtils.isEmpty(uiConversationMsg.getWkChannel().channelRemark)) {
            if (isEnd) {
                sortMsg(chatConversationAdapter.getData());
            }
            return;
...
//                                    chatConversationAdapter.notifyItemChanged(i, chatConversationAdapter.getData().get(i));
    ...
    public void onDestroy() {
        ...
        try {
            ...
            Log.e("ChatFragment", "onDestroy remove listeners error: " + e.getMessage(), e);
        }
    }
...
//        } else pcLoginTv.setText(appLoginType);
    ...
    private void updateTitleByConnectionStatus(int status) {
        if (wkVBinding == null) return;
        if (status == WKConnectStatus.success || status == WKConnectStatus.syncCompleted) {
            wkVBinding.titleTv.setText(R.string.app_name);
        } else if (status == WKConnectStatus.syncMsg) {
            wkVBinding.titleTv.setText("收取中…");
        } else if (status == WKConnectStatus.kicked) {
            wkVBinding.titleTv.setText("已断开");
        } else {
            wkVBinding.titleTv.setText("连接中…");
        }
    }
