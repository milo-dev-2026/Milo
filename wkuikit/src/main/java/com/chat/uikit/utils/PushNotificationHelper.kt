...
object PushNotificationHelper {
    ...
    /** 通知渠道-聊天消息(重要性级别-高：发出声音，并以浮动通知的形式显示) */
    ...
    fun notifyCall(
        ...
        val intent = Intent(context, TabActivity::class.java)
        val callChannel = if (playSound && vibrate) {
            CALL
        } else {
            CALL.copy(
                sound = if (playSound) CALL.sound else null,
                vibrate = if (vibrate) CALL.vibrate else null
            )
        }
        val builder = NotificationCompatUtil.createNotificationBuilder(
            context,
